...
export default function Home() {
  return <>
    ...
    <main>
      ...
      <section className="routes" id="routes"><div className="shell"><p className="eyebrow">High-demand SNA routes</p><h2>John Wayne Airport transportation across Southern California</h2><div className="routeGrid">{popularRoutes.map(r=><Link href={`/routes/${r.slug}`} className="routeCard" key={r.slug}><span>SNA →</span><h3>{r.city}</h3><p>{r.copy}</p><b>View route <ArrowRight size={15}/></b></Link>)}</div><div className="allCitiesCta"><Link className="btn dark" href="/service-areas">View all {ROUTES.length} city service areas <ArrowRight size={17}/></Link></div></div></section>
      <section className="visualCta" style={{backgroundImage:`linear-gradient(90deg,rgba(6,11,23,.94),rgba(6,11,23,.55)),url("${aircraft}")`}}><div className="shell"><p className="eyebrow light">Private airport transportation</p><h2>Landing at SNA?<br/>Your chauffeur can be ready.</h2><p>Reserve your airport transfer in advance for a calmer arrival and a direct ride to your Orange County destination.</p><a className="btn primary" href={BOOKING_URL} target="_blank" rel="noreferrer">Get a quote <ArrowUpRight size={18}/></a></div></section>
      <section className="why shell" id="why"><p className="eyebrow">Why travelers choose reserved service</p><h2>Professional airport transportation without the uncertainty.</h2><div className="whyGrid"><div><strong>01</strong><h3>Private pickup</h3><p>Your reservation is planned for your party—not shared with strangers.</p></div><div><strong>02</strong><h3>Local expertise</h3><p>Orange County routes, airport access and timing are part of the plan.</p></div><div><strong>03</strong><h3>Direct service</h3><p>From the airport to hotels, homes, meetings, resorts and events.</p></div><div><strong>04</strong><h3>Professional support</h3><p>Real reservation assistance for changes, groups and special requests.</p></div></div></section>
    </main>
    <footer><div className="shell footerGrid"><div><Brand/><p>Private airport car service serving John Wayne Airport and Southern California.</p></div><div><b>Reservations</b><a href={PHONE_HREF}>{PHONE_DISPLAY}</a><a href={BOOKING_URL} target="_blank" rel="noreferrer">Book online</a></div><div><b>Service areas</b><Link href="/service-areas">Orange County cities</Link><Link href="/service-areas">Los Angeles County cities</Link></div></div><div className="shell credits">Recent John Wayne Airport photography: Famartin, Wikimedia Commons, CC BY-SA 4.0. Site operates as a South Coast Limousines & Transportations, Inc. service.</div></footer>
  </>;
}
