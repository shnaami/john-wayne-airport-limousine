...
<section className="fleetSection shell"><div className="fleetIntro"><p className="eyebrow">Luxury vehicles for SNA & Orange County</p><h2>Exterior and interior views of our fleet.</h2><p>Each vehicle is shown with its matching exterior and interior. Vehicle availability varies by date; we’ll help match your passenger count, luggage and service requirements.</p></div><div className="fleetList">{vehicles.map((v,i)=><article className="fleetVehicle" key={v.name}><div className="fleetPhotos"><div className={`fleetPhoto ${v.name === "Cadillac Escalade" ? "escaladeExterior" : ""}`}><img src={v.exterior} alt={`${v.name} exterior for John Wayne Airport limousine service`}/></div><div className="fleetPhoto"><img src={v.interior} alt={`${v.name} interior for John Wayne Airport private transportation`}/></div></div><div className="fleetInfo"><span className="fleetNumber">0{i+1}</span><p className="eyebrow">{v.type}</p><h2>{v.name}</h2><div className="capacity"><Users size={19}/><strong>{v.capacity}</strong></div><p>{v.copy}</p><a className="btn dark" href={BOOKING_URL} target="_blank" rel="noreferrer">Reserve this vehicle <ArrowUpRight size={17}/></a></div></article>)}</div></section>
